...
| 편집 화면 | edit view | 표시 격자·블록형 배치 (D-003, D-010) | |
...
| 한국어 | ID / 영문 | 정의 | 쓰지 말 것 |
|---|---|---|---|
| 논리 셀 | logic cell | 배치·판정·길찾기·저장의 최소 단위. **한 변 0.5cm** (D-010). 측면 뷰는 가로×높이 평면 | 도트 셀, 스프라이트 셀과 동일시 |
| 논리 격자 | logic grid | 논리 셀의 집합 (D-003, D-010) | |
| 표시 격자 | display grid | 편집 화면에 그리는 선. 가독성용으로 N칸마다일 수 있음. **스냅 단위는 논리 셀** | 표시 간격 = 논리 셀이라고 단정 |
...
| 논리 셀 (0.5cm) | 스프라이트 셀 (px) | 배치·판정 vs 아트 캔버스 |
